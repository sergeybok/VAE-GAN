...
class simpleGAN:
	...
	def build_gan(self,optimizer=tf.train.AdamOptimizer):
		...
		#gen
		layer_1, layer_1_params = dense_layer(self.Z,[None,self.latent_dim],[None,4096],activation=tf.nn.elu,name='gen_1')
		self.generated, layer_2_params = dense_layer(layer_1,[None,4096],[None,784],activation=tf.nn.sigmoid,name='gen_2')
		self.gen_params = layer_1_params + layer_2_params


		#dis
		d_in = tf.contrib.layers.flatten(self.X)
		dlayer1, dlayer1_params = dense_layer(d_in,[None,784],[None,1024],activation=tf.nn.elu,name='dis_1')
		self.data_prediction, dlayer2_params = dense_layer(dlayer1,[None,1024],[None,1],activation=tf.nn.sigmoid,name='dis_2')

		dlayer1_, _ = dense_layer(self.generated,[None,784],[None,1024],activation=tf.nn.elu,weight=dlayer1_params[0],bias=dlayer1_params[1])
		self.gen_prediction, _ = dense_layer(dlayer1_,[None,1024],[None,1],activation=tf.nn.sigmoid,weight=dlayer2_params[0],bias=dlayer2_params[1])
			...
			for batch in range(n_batches):
				...
				if not stabilize or ((sum(d_list[-stabilize_batch:])/stabilize_batch) > 0.2):
					_, dcost = self.sess.run([self.train_step_d, self.cost_d],
							...
							#feed_dict={self.X:tx,self.Z:tz,self.keep_prob:keep_prob,self.LR:lr,self.phase:True})
					prev_dcost = dcost.mean()
				else:
					dcost = self.sess.run([self.cost_d],
							feed_dict={self.X:tx.reshape((-1,28,28,1)),self.Z:tz,self.keep_prob:keep_prob,self.phase:True})
							#feed_dict={self.X:tx,self.Z:tz,self.keep_prob:keep_prob,self.phase:True})
					stable_d +=1
				...
				if not stabilize or ((sum(d_list[-stabilize_batch:])/stabilize_batch) < 1.5):
					...
					_, gcost = self.sess.run([self.train_step_g,self.cost_g],
							...
							#feed_dict={self.X:tx,self.Z:tz,self.keep_prob:keep_prob,self.LR:(lr/2.0),self.phase:True})
					stable_g +=1
				d_list.append(np.mean(dcost))
				g_list.append(np.mean(gcost))
	...
	def build_vae(self,optimizer=tf.train.AdamOptimizer):
		...
		layer_1, _ = dense_layer(decoder_in,[None,self.latent_dim],[None,4096],activation=tf.nn.elu,weight=self.gen_params[0],bias=self.gen_params[1],name='gen_1')
		self.decoded, _ = dense_layer(layer_1,[None,4096],[None,784],activation=tf.nn.sigmoid,weight=self.gen_params[2],bias=self.gen_params[3],name='gen_2')

		self.klloss = -(1)*tf.reduce_sum(1 + tf.log(self.vae_sigma**2) - self.vae_mu**2 - self.vae_sigma**2,0)
		self.klloss = tf.reduce_mean(self.klloss)/50
		...
		self.vae_cost = tf.reduce_mean(self.logloss + self.klloss)
